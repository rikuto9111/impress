...
//検索システム
/*
 @Observable class こうしないと外部のstructからこのクラスを参照するときこのclassのフィールドを
 メソッドとか通じて変化させるとする。classのフィールドが変化 -> classのフィールド値を外部structで
 表示させる。この時その変化は当たり前のように瞬時に追跡できてると思うだろうけどそれができませーーん
 
 
 検索システムを実装するとき
1  必ず本体メソッドはTask(非同期空間){}の中に入れてね! そうじゃないと検索が始まってから終わるまで何か検知したりとか何もできなくなる

2  await 本体メソッドするようにする
 本体メソッドの中でURLsession(API)を使うのであればasyncな動きであるのでasyncなものがメソッドに入ってる時は必ずそのメソッドはawaitをつける
 
 基本的なawaitの意味は次の行の処理に進まずにawaitがついているメソッドが終わるまで待つって感じ
 
3  バックグラウンドで実行するメソッド(async)に対して MainActorをつける

 もしつけないと今回の場合はビュー形成にゴリゴリ影響のある状態変数を触ることになるからこいつの更新は必ず
 mainスレッドで行わなければならないからMainActorをつけることで必ずmainスレッドで実行しますよっていう保証をする
 
4  とりあえず
 URL(string:) URLをまずは作る
 URLsession.shared.dataでそのURLを元にこの非同期な通信を行い(await)をつけてやる
 その結果を(data,_)で受け取る -> デコードする -> JSondata().decode(structの型(Codable),from data) Codableな型を作ってデコードするとき全部じゃなくて
 型の中にある変数(実際のJsonの中の変数名と同じにする)と同じものだけ取り出す(入れ子構造とかもjsonと合わせる)-> 格納
 
 5 今回はcollectionに対して後々ForEachとかで使いたいし入れ子とかやめて使いやすくしたいからidを追加したりして
 structを別に作っておいてそこにCodable型からどんどん嵌めていく
 
 6 そのstruct構造の配列をクラスフィールドとして外部から参照する
 */
