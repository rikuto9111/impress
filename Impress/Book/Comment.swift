...
struct Comment: View {
    ...
    @State var isProfilenavigation = false
    
    @State var isotProfilenavigation = false
    
    @State var otuserid = ""
    @State var otregisterday = Timestamp(date:Date())
    
    @Environment(\.presentationMode) var presentationMode

    var body: some View {
        
        NavigationLink(destination:Otherprofile(othuserid:book.othUserId,registerday: book.registerday),isActive: $isProfilenavigation){
            
        }
        NavigationLink(
            destination:Otherprofile(othuserid: otuserid, registerday: otregisterday),isActive: $isotProfilenavigation){}
        ...
        ZStack{
            VStack{
                ...
                HStack{
                    Button(action:{isProfilenavigation = true}){
                ...
                if istouchButton{
                    ...
                    ForEach(book.isComment,id:\.self){ comment in
                        ...
                        HStack{
                            ...
                            if let url = comment["icon"],let ouserid = comment["docuserid"],let oregisterday = comment["otregisterday"]{
                                ...
                                Button(action:{
                                    otuserid = ouserid
                                    if let date = formatter.date(from:oregisterday){
                                        let timestamp = Timestamp(date: date)
                                        
                                        otregisterday = timestamp
                                        print(oregisterday)
                                        print(otuserid)
                                        
                                        DispatchQueue.main.async{
                                            isotProfilenavigation = true
                                        }
                                    }
            ...
            if isShown{
                ...
                CustomDialog6(isShown:$isShown,isEdit:$isEdit,onSave:
                                {
                    let dating = Date()
                    let day = formatter.string(from: dating)
                    
                    updateEdit(isComment:isEdit,othUserId:book.othUserId,documentId:book.documentId,date:day)
    ...
    private func updateEdit(isComment:String,othUserId:String,documentId:String,date:String){
        ...
        let month = calendar.component(.month, from: registerday)//月を取り出してくれるツール
        
        let year = calendar.component(.year, from: registerday)//月を取り出してくれるツール
        let day = calendar.component(.day, from: registerday)//月を取り出してくれるツール
        ...
        db.collection("user")
            ...
            .getDocument{snapshot,error in
                ...
                let newComment:[String:String] = ["text":isComment,"icon":icon,"name":name,"date":date,"docuserid":UID,"otregisterday":"\(year)年\(month)月\(day)日"]
                ...
                book.isComment.append(newComment)
            }
    }
    
    private func updateGoodness(isGood:Bool,othUserId:String,documentId:String,goodness:Int){
       var db = Firestore.firestore()
...
struct CustomDialog6: View {//こいつを呼び出すだけでviewを表示するんだね
